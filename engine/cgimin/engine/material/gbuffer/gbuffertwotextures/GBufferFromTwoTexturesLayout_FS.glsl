...
void main()
{
	gColorRoughness = texture(color_texture, texcoord);
	gPosition = position;
	
	vec4 normalMetalness = texture(normalmap_texture, texcoord).rgba;
	...
	gMetalnessShadow.r = normalMetalness.a;
	gMetalnessShadow.g = visibility;

	vec3 normal = normalize(normalMetalness.rgb * 2.0 - 1.0); 
