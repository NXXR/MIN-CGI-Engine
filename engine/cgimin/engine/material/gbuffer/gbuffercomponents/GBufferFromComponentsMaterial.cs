...
{
    public class GBufferFromComponentsMaterial : BaseMaterial
    {
        ...
        private int metalnessTextureLocation;
        private int roughnessTextureLocation;
        private int aoTextureLocation;
        private int glowTextureLocation;
        ...
        public GBufferFromComponentsMaterial()
        {
            // Shader-Programm wird aus den externen Files generiert...
            CreateShaderProgram("cgimin/engine/material/gbuffer/gbuffercomponents/GBufferFromComponents_VS.glsl",
                                "cgimin/engine/material/gbuffer/gbuffercomponents/GBufferFromComponents_FS.glsl");
        ...
        public void Draw(BaseObject3D object3d, int colorTextureID, int normalTextureID, int metalnessTextureID, int roughnessTextureID, int aoTextureLocation, int glowTextureLocation)
        {
            ...
            GL.BindTexture(TextureTarget.Texture2D, metalnessTextureID);

            GL.ActiveTexture(TextureUnit.Texture3);
            GL.BindTexture(TextureTarget.Texture2D, roughnessTextureID);

            GL.ActiveTexture(TextureUnit.Texture4);
            GL.BindTexture(TextureTarget.Texture2D, aoTextureLocation);

            GL.ActiveTexture(TextureUnit.Texture5);
            GL.BindTexture(TextureTarget.Texture2D, glowTextureLocation);
            ...
            GL.Uniform3(lightDirectionLocation, CascadedShadowMapping.LightDirection);
        ...
        {
            Draw(object3d, settings.colorTexture, settings.normalTexture, settings.metalnessTexture, settings.roughnessTexture, settings.aoTexture, settings.glowTexture);
        }

        public void DrawDirect(BaseObject3D object3d, int colorTextureID, int normalTextureID, int metalnessTextureID, int roughnessTextureID, int aoTextureLocation, int glowTextureLocation)
        {
            PreDraw();
            Draw(object3d, colorTextureID, normalTextureID, metalnessTextureID, roughnessTextureID, aoTextureLocation, glowTextureLocation);
            PostDraw();
        }
